...
void print_ff(unsigned char *msg, int len, struct sockaddr_in6 *addr, int hoplimit, unsigned int if_index, int edefs)
{
	...
	while (orig_len > 0) {
		...
		case ND_OPT_PREFIX_INFORMATION:
			...
			print_addr(&pinfo->nd_opt_pi_prefix, prefix_str, sizeof(prefix_str));
		...
		case ND_OPT_RDNSS_INFORMATION:
			...
			print_addr(&rdnss_info->nd_opt_rdnssi_addr1, prefix_str, sizeof(prefix_str));
			printf(" %s", prefix_str);

			if (rdnss_info->nd_opt_rdnssi_len >= 5) {
				print_addr(&rdnss_info->nd_opt_rdnssi_addr2, prefix_str, sizeof(prefix_str));
				printf(" %s", prefix_str);
			}
			if (rdnss_info->nd_opt_rdnssi_len >= 7) {
				print_addr(&rdnss_info->nd_opt_rdnssi_addr3, prefix_str, sizeof(prefix_str));
				printf(" %s", prefix_str);
			}
