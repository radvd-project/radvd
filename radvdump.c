/*
 *   $Id: radvdump.c,v 1.4 2000/11/26 22:17:12 lf Exp $
 *
 *   Authors:
...
void print_ff(unsigned char *, int, struct sockaddr_in6 *, int);
...
main(int argc, char *argv[])
{
	...
	/* parse args */
#ifdef HAVE_GETOPT_LONG
	while ((c = getopt_long(argc, argv, "d:fhv", prog_opt, &opt_idx)) > 0)
#else
	while ((c = getopt(argc, argv, "d:fhv")) > 0)
#endif
	{
		switch (c) {
		case 'd':
	...
	for(;;)
	{
			...
			else if (icmph->icmp6_type == ND_ROUTER_ADVERT)
			{
				if (!fout)
					...
					print_ff(msg, len, &rcv_addr, hoplimit);
			}
        	}
		else if (len == 0)
       	 	{
...
print_ra(unsigned char *msg, int len, struct sockaddr_in6 *addr, int hoplimit)
{
	...
	while (len > 0)
	{
		...
		case ND_OPT_RTR_ADV_INTERVAL:
			...
			printf("\t\tAdvInterval: %ld", (ntohl(a_ival->adv_ival)) / 1000);
			printf("\n");
			break;
		/* Mobile IPv6 ext */
		case ND_OPT_HOME_AGENT_INFO:
			...
			printf("\t\tHomeAgentPreference: %ld", ntohs(ha_info->preference));
			printf("\n");
			printf("\t\tHomeAgentLifetime: %ld", ntohs(ha_info->lifetime));
			printf("\n");
			break;
...
void
print_ff(unsigned char *msg, int len, struct sockaddr_in6 *addr, int hoplimit)
{
	struct nd_router_advert *radvert;
	char addr_str[INET6_ADDRSTRLEN];
	uint8_t *opt_str;
	...
	print_addr(&addr->sin6_addr, addr_str);
	printf("#\n# Configuration generated by radvdump\n");
	...
	while (len > 0)
	{
		...
		case ND_OPT_HOME_AGENT_INFO:
			...
			printf("\tHomeAgentPreference %ld;\n", ntohs(ha_info->preference));
			printf("\tHomeAgentLifetime %ld;\n", ntohs(ha_info->lifetime));
			break;
		case ND_OPT_TARGET_LINKADDR:
		case ND_OPT_REDIRECTED_HEADER:
			log(LOG_ERR, "invalid option %d in RA", (int)*opt_str);
