...
static void add_option_prefix(struct safe_buffer * sb, struct Interface const * iface, char const * ifname, struct AdvPrefix const * prefix, int cease_adv);
static void add_option_mtu(struct safe_buffer * sb, uint32_t AdvLinkMTU);
static void add_option_sllao(struct safe_buffer * sb, struct sllao const *sllao);
static void add_option_mipv6_rtr_adv_interval(struct safe_buffer * sb, double MaxRtrAdvInterval);
static void add_option_mipv6_home_agent_info(struct safe_buffer * sb, struct mipv6 const * mipv6);
static void add_option_lowpanco(struct safe_buffer * sb, struct AdvLowpanCo const *lowpanco);
static void add_option_abro(struct safe_buffer * sb, struct AdvAbro const *abroo);

// Options that generate 0 or more blocks
static struct safe_buffer_list* add_options_prefix(struct safe_buffer_list * sbl, struct Interface const * iface, char const * ifname, struct AdvPrefix const * prefix, int cease_adv, struct in6_addr const *dest);
static struct safe_buffer_list* add_options_route(struct safe_buffer_list * sbl, struct Interface const * iface, struct AdvRoute const *route, int cease_adv, struct in6_addr const *dest);
static struct safe_buffer_list* add_options_rdnss(struct safe_buffer_list * sbl, struct Interface const * iface, struct AdvRDNSS const *rdnss, int cease_adv, struct in6_addr const *dest);
static struct safe_buffer_list* add_options_dnssl(struct safe_buffer_list * sbl, struct Interface const * iface, struct AdvDNSSL const *dnssl, int cease_adv, struct in6_addr const *dest);
	...
	if (get_v4addr(prefix->if6to4, &dst) < 0) {
		...
		*((uint16_t *)(xprefix.Prefix.s6_addr)) = htons(0x2002);
		...
		if(cease_adv || schedule_option_prefix(dest, iface, &xprefix)) {
			...
			sbl = sbl->next;
			add_option_prefix(sbl->sb, iface, ifname, &xprefix, cease_adv);
		}
	}
#endif
	return sbl;
		...
		if ((!prefix->DecrementLifetimesFlag || prefix->curr_preferredlft > 0)) {
			...
			if (prefix->if6to4[0] || prefix->if6[0] || 0 == memcmp(&prefix->Prefix, &zero, sizeof(zero))) {
				...
				if(cease_adv || schedule_option_prefix(dest, iface, prefix)) {
					sbl->next = new_safe_buffer_list();
					sbl = sbl->next;
					add_option_prefix(sbl->sb, iface, ifname, prefix, cease_adv);
				}
			}
		}
...
static struct safe_buffer_list * build_ra_options(struct Interface const * iface, struct in6_addr const *dest)
{
	...
	if (iface->AdvPrefixList) {
		cur = add_options_prefix(cur, iface, iface->props.name, iface->AdvPrefixList, iface->state_info.cease_adv, dest);
	}

	if (iface->AdvRouteList) {
		cur = add_options_route(cur, iface, iface->AdvRouteList, iface->state_info.cease_adv, dest);
	}

	if (iface->AdvRDNSSList) {
		cur = add_options_rdnss(cur, iface, iface->AdvRDNSSList, iface->state_info.cease_adv, dest);
	}

	if (iface->AdvDNSSLList) {
		cur = add_options_dnssl(cur, iface, iface->AdvDNSSLList, iface->state_info.cease_adv, dest);
	}

	if (iface->AdvLinkMTU != 0 && schedule_option_mtu(dest, iface)) {
		cur->next = new_safe_buffer_list();
		cur = cur->next;
		add_option_mtu(cur->sb, iface->AdvLinkMTU);
	}

	if (iface->AdvSourceLLAddress && iface->sllao.if_hwaddr_len > 0 && schedule_option_sllao(dest, iface)) {
		cur->next = new_safe_buffer_list();
		cur = cur->next;
		add_option_sllao(cur->sb, &iface->sllao);
	}

	if (iface->mipv6.AdvIntervalOpt && schedule_option_mipv6_rtr_adv_interval(dest, iface)) {
		cur->next = new_safe_buffer_list();
		cur = cur->next;
		add_option_mipv6_rtr_adv_interval(cur->sb, iface->MaxRtrAdvInterval);
	}
	...
	if (iface->mipv6.AdvHomeAgentInfo && schedule_option_mipv6_home_agent_info(dest, iface)
		...
		cur->next = new_safe_buffer_list();
		cur = cur->next;
		add_option_mipv6_home_agent_info(cur->sb, &iface->mipv6);
	}

	if (iface->AdvLowpanCoList && schedule_option_lowpanco(dest, iface)) {
		cur->next = new_safe_buffer_list();
		cur = cur->next;
		add_option_lowpanco(cur->sb, iface->AdvLowpanCoList);
	}

	if (iface->AdvAbroList && schedule_option_abro(dest, iface)) {
		cur->next = new_safe_buffer_list();
		cur = cur->next;
		add_option_abro(cur->sb, iface->AdvAbroList);
	}
	...
	*/
	...
	do {
		...
		if(option_count == 0) {
			...
			dlog(LOG_DEBUG, 5, "No RA options scheduled in this pass, staying quiet");
			break;
		}
